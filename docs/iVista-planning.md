...
plugins/ivista/skills/ivista/SKILL.md
...
- WDA fork 不放进 npm 包。
- WDA fork 不放进 Codex Plugin bundle。
...
这个 tag 当前应视为“原生 WDA 基线版本”。后续原生 UI、图标、状态页等定制完成后，应打新的 tag，例如：

```text
ivista-wda-v0.2.0
```

然后再把 `src/ivista-runtime.mjs` 里的默认 WDA ref 升级到新 tag。
...
- CLI 是核心入口。
- Codex Plugin 只做 thin skill，指导 Agent 安装和调用 CLI。
...
src/                  modular CLI runtime: core/devices/wda/actions/sessions/doctor
plugins/ivista/       Codex Plugin metadata + install/use Skills only
...
- 能打开和关闭指定 App。
- 能输出 JSON。
- 能通过 Codex Plugin Skill 指导 Agent 调用 CLI。
...
4. 外层 iVista 把默认 WDA ref 从 `ivista-wda-v0.1.1` 升到新 tag。
5. 发新的 CLI 版本。
...
- 当前默认 ref 仍是 `ivista-wda-v0.1.1`，除非后续已更新。
- 当前 WDA UI 改动需要新 tag 才会进入默认用户路径。
