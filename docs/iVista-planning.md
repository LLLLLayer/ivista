...
- Simulator 可以更快验证 CLI、WDA Client、MCP、截图、source、tap/input/swipe、App 启停等核心闭环。
...
MCP 用来给 Codex、Claude Code、Trae 等 Agent 暴露结构化工具。
...
- `ivista_wait`
- `ivista_logs`
- `ivista_report_append`
- `ivista_run_recipe`

MCP 层不应该重新实现设备逻辑，只调用 CLI 或 runtime SDK。
...
- MCP server
- Codex/Claude Code 可用的 Skill 文档
- 基础 HTML/Markdown 报告

MVP 的推荐验证顺序是先完成自动管理 WDA 的 Simulator，再做真机连接验证：

- Simulator WDA：自动下载/缓存 WDA，验证 runtime、CLI、MCP、截图、source 和动作执行。
