...
The pinned release for this skill is `v1.0.9`. The skill and CLI should normally use the same iVista release because command examples and behavior may change across releases.

If `ivista version` is missing or older than `iVista CLI 1.0.9`, update the CLI before continuing:
...
If `ivista version` is newer than `iVista CLI 1.0.9`, the plugin skill is probably stale. Update the plugin/skill to the matching CLI release, or install the latest plugin release and rerun this check.
...
- `ivista` missing: install with `npm install -g git+https://github.com/LLLLLayer/ivista.git#v1.0.9`.
- CLI older than this skill: run `ivista update --ref v1.0.9`, or reinstall from the pinned GitHub tag if update fails.
